...
public class DemandeController {
    ...
    @PostMapping
    public ResponseEntity<?> ajouter_demande(@RequestBody DemandeForAnnonceDto demandeDto){
        long id_annonce = demandeDto.getId_annonce();
        long id_demandeur = demandeDto.getId_demmandeur();
        boolean response = demandeService.faire_demande_pour_annonce(id_annonce, id_demandeur);

        if (!response) {
            return ResponseEntity.ok().build(); // Returns a 200 OK status
        } else {
            return ResponseEntity.badRequest().build(); // Returns a 400 Bad Request status
        }
    }
