 ...
 */
    ...
    @Override
    public List<FiltreAnnonceResponse> getAllAnnoncesPublieIncludeAnnonceurid() {
        List<Annonce> allAnnonces=annonceRepository.findAll();
        List<Annonce> Annonce_publie=new ArrayList<>();
        for(int i=0;i<allAnnonces.size();i++) {
            if (allAnnonces.get(i).getEtat().equals(Etat.PUBLIEE)) {
                Annonce_publie.add(allAnnonces.get(i));
            }
        }
        List<FiltreAnnonceResponse> AnnoncesPublieIncludeAnnonceuridList = new ArrayList<>();

        for (Annonce annonce : Annonce_publie) {
            ...
            AnnoncesPublieIncludeAnnonceuridList.add(filtreAnnonceResponse);
        }
        return AnnoncesPublieIncludeAnnonceuridList;
    }
    ...
    @Override
    public List<FiltreAnnonceResponse> filtrerAnnoncesPublieeMobile(String typeBien, String operationType, String minSurface, String maxSurface, String minPrix, String maxPrix, Double commune_id) {
        List<Annonce> annonces = annonceRepository.filtrerAnnoncesPublic(typeBien,operationType,minSurface,maxSurface,minPrix,maxPrix,commune_id,Etat.PUBLIEE);
        List<FiltreAnnonceResponse> filtreAnnoncesResponseList = new ArrayList<>();
